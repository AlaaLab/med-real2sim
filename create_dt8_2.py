...
vs = np.linspace(-20., 350., 6)
startps = np.linspace(40., 90., 3)
rcs = np.linspace(0.08, 5., 1)
emaxs = np.linspace(0.3, 8., 5)
emins = np.linspace(0.02, 0.1, 4)
vds = np.linspace(4., 16., 1)
cas = np.linspace(0.08, 3.5, 1)
rss = np.linspace(0.5, 2., 4)
css = np.linspace(1., 1.6, 4)
...
for i in range(N_test):
  ...
  x_test_tc[i][1] = random.uniform(-20., 350.)
  x_test_tc[i][2] = random.uniform(40., 90.)
  x_test_tc[i][3] = random.uniform(0.3, 8.)
  x_test_tc[i][4] = random.uniform(0.02, 0.1)
  x_test_tc[i][5] = random.uniform(0.5, 2.)
  x_test_tc[i][6] = random.uniform(1., 1.6)
...
def pvloop_simulator(Tc, start_v, startp, Emax, Emin, Vd, Rs, Cs):
    ...
    if (ped > 3. and ped < 15 and pes < 150. and pes > 80): isrealistic_2 = 1
