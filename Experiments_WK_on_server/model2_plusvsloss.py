...
class CardioDataset(torch.utils.data.Dataset):
    def __init__(self, patient_ids, sequences, edvs, esvs, efs, vss):
...
train_data = CardioDataset(patient_ids=patient_id, sequences = sequences_all, edvs = LVedv, esvs= LVesv, efs = LVef, vss = VS)
...
model = NEW3DCNN(num_parameters = 12)
model.to(device)
...
for epoch in range(num_epochs):
    ...
    for j, batch in enumerate(train_loader):
        ...
        for i in range(batch_size):
           ...
           svloss = criterion(0.2+0.15*x[0][0], true_SV)/0.3*100/0.3*100
...
output_file = os.path.join(output_path, 'model2_plus_sv_loss50e300r.csv')  # set the output file path

# save the data to the output file
np.savetxt(output_file, np.column_stack((sim_EF_array, trueEF_array, sim_EDV_array, trueEDV_array, sim_ESV_array, trueESV_array)), delimiter=',', header='output_model2_loss2')
