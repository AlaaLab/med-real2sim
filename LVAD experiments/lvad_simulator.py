...
def f_nolvad():
    ...
    N = 50
    global Tc, start_v, start_pao, Emax, Rs, Rm, Ra, Rc, Ca, Cs, Cr, Ls, Vd, Emin
    ...
    return result_Vlv[18*60000:20*60000], result_Plv[18*60000:20*60000], ef, ((ved - ves) * 60/Tc ) / 1000, sol[19*60000, 0], sol[19*60000, 1], sol[19*60000, 2], sol[19*60000, 3], sol[19*60000, 4]

#returns the w at which suction occurs: (i.e. for which the slope of the envelopes of x6 becomes negative)
def get_suctionw(y00, y01, y02, y03, y04, w0, x60, ratew): #slope is slope0 for w

    N = 70
    global Tc, Emax, Rs, Rm, Ra, Rc, Ca, Cs, Cr, Ls, Vd, Emin
...
def f_lvad(c, slope, w0, x60, y00, y01, y02, y03, y04): #slope is slope0 for w
    ...
    for j in range(0, n-1):
      ...
      #update the min of x6 in the current cycle. also keep the time at which the min is obtained (for getting the slope later)
      if (minx6 > sol[j][5]):
        minx6 = sol[j][5]
        tmin = t[j]
...
##HOW TO SIMULATE AN LVAD INTERVENTION:

## CIRCUIT PARAMETERS:

#learnable in 7par model:
Tc = 1.
start_v = 140.
Emax = 2.
Emin = 0.06
...
# 1. GET VALUES OF INITIAL SYSTEM WHEN ALREADY IN PERIODIC STATE:
vlvs, plvs, ef_nolvad, co_nolvad, y00, y01, y02, y03, y04 = f_nolvad()

## 2. GET SUCTION SPEED (ie suctionw): (make w go linearly from w0 to w0 + maxtime * 400, and find w at which suction occurs)
w0 = 5000.
x60 = 250.
ratew = 400.
suctionw = get_suctionw(y00, y01, y02, y03, y04, w0, x60, ratew)

## 3. SET PARAMETERS FOR LVAD:
...
## 4. COMPUTE NEW PV LOOPS, EF, CO, MAP WITH CIRCUIT+LVAD:
new_ef, CO, MAP = f_lvad(c, slope0, w0, x60, y00, y01, y02, y03, y04) # already plots the new pvloops

print("Suction speed:", suctionw)
print("Before LVAD: EF:", ef_nolvad, "CO:", co_nolvad)
print("With LVAD: EF:", new_ef, "CO:", CO, "MAP:", MAP, "\n\n")
