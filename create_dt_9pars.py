...
def f(Tc, start_v, startp, Rc, Emax, Emin, Vd, Ca, Rs):
        ...
        for Rc in rcs:
          for Emax in emaxs:
            for Emin in emins:
              for Ca in cas:
                for Rs in rss:
                  vedest, vesest, minv, minp = f(Tc, start_v, startp, Rc, Emax, Emin, Vd, Ca, Rs)
                  vedssim.append(vedest)
                  vesssim.append(vesest)
                  pts.append([Tc, start_v, startp, Rc, Emax, Emin, Vd, Ca, Rs])
                  if (minv<=0 or minp<=0): 
                    errors.append(i)
                    print("Error", i)
                  if (i%1000==0): print(i)
...
for i in range(N_test):
  ...
  x_test_tc[i][3] = random.uniform(0.08, 0.1)
  x_test_tc[i][4] = random.uniform(0.2, 8.)
...
def pvloop_simulator(Tc, start_v, startp, Rc, Emax, Emin, Vd, Ca, Rs):
...
ved, ves, minv, minp = pvloop_simulator(1., 100., 50., 0.07, 3., 0.08, 5., 0.08, 1.)
