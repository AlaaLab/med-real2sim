# -*- coding: utf-8 -*-
'''
Estimates some of the parameters of the model and the initial conditions: 
(Tc, Rs, Rc, Cs, start_v, start_pao, Emax, Emin) (assuming start_pa=start_pao and start_Qt=0, and start_pla = start_plv - 0.2).
Vd has no effect on the equations so cannot be estimated.
'''
...
parser.add_argument('--int_time', type=float, default=3.1) #added
...
class IntervalNormalizationLayer(torch.nn.Module):
    def __init__(self):
        super().__init__()
        #output = (Tc, Rs, Rc, Ca:0.0800, Cs, Cr:4.4000, Ls:0.0005, start_v, start_pao, Emax, Emin). start_v: 20-400; start_pla: 5-20, start_pao: 50-100.
        #output = (Tc, Rs, Rc, Cs, start_v, start_pao, Emax, Emin)
        self.a = torch.tensor([0.5, 0.2, 0.01, 0.2, 30, 30, 0.3, 0.01], dtype=torch.float32) #HR in 20-200->Tc in [0.3, 4]
        self.b = torch.tensor([1.5, 4.0, 0.15, 2.5, 500, 120, 7.0, 0.40], dtype=torch.float32)
...
class NEW3DCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = (Tc, Rs, Rc, Cs, start_v, start_pao, Emax, Emin)

        Rm = torch.tensor(0.0050)
        Ra = torch.tensor(0.0010)
        Ca = torch.tensor(0.0800)
        Cr = torch.tensor(4.4000)
        Ls = torch.tensor(0.0005)
        start_pla = x[0][4] * x[0][7] - torch.tensor(0.2) # = Elastance(t=0) * start_v - 0.2; E(0)=Emin=0.06

        ini_conds = torch.tensor([x[0][4], start_pla, x[0][5], x[0][5], torch.tensor(0)]) #start_pao = start_pa = x[0][8]

        func = ODEFunc(x[0][0], x[0][1], Rm, Ra, x[0][2], Ca, x[0][3], Cr, Ls, x[0][6], x[0][7]).to(device)
    
        pred_y = odeint(func, ini_conds, t, atol=1e-6, rtol=1e-6).to(device)

        #changed to predict: Vlv(2Tc) (ED), and Vlv(0.2+0.15Tc + 2Tc) 
        sim_edv = pred_y[int(2 * x[0][0] * args.data_size / args.int_time)][0] + 10 #pred_y[n][0][m]: n frame, m-th component (m=0: Vlv-Vd)
        sim_esv = pred_y[int((0.2+1.15*x[0][0])*args.data_size / args.int_time)][0] + 10
...
class ODEFunc(nn.Module): #takes y, t and returns dydt = NN(y,t). NN: defined in self.net (NN: (y**3, t**3):2D->50->tanh->2D=dydt)
    ...
    def __init__(self, Tc, Rs, Rm, Ra, Rc, Ca, Cs, Cr, Ls, Emax, Emin): #cnn: NEW3DCNN
...
for epoch in range(num_epochs):
    for i, batch in enumerate(train_loader):
        ...
        print("sim_edv", sim_edv, "trueedv", trueV_ED)
        #print("sim_esv", sim_esv, "trueesv", trueV_ES)
